...
export function Sheet({ aberto, titulo, sub, onFechar, children }: SheetProps) {
  useEffect(() => {
    if (!aberto) return;
  ...
  return (
    <>
      ...
      <div
        ...
        aria-modal="true"
        aria-label={titulo}
        className={`fixed inset-x-0 bottom-0 z-50 max-h-[88vh] overflow-y-auto rounded-t-2xl bg-surf transition-transform duration-250 ease-out ${aberto ? "translate-y-0" : "translate-y-full"}`}
