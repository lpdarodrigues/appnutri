...
export function Diario() {
  ...
  const ordenadas = d ? d.meals.map((m, i) => ({ m, i })).sort((a, b) => (a.m.h || "99:99").localeCompare(b.m.h || "99:99")) : [];

  return (
    <>
      {/* Cabeçalho fixo com o consumo do dia */}
            ...
            {ordenadas.map(({ m, i }) => {
              ...
              return (
                <div key={i} className={`card overflow-hidden transition-opacity ${on ? "opacity-60" : ""}`}>
                  <button onClick={() => alternar(i)} aria-expanded={op} className="w-full px-4 pt-3.5 pb-3 text-left">
